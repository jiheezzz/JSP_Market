...
<input  type=hidden  name=sw  value="MS">
<select name=str1>
...
<div align="center">
<table  border=1 >
<tr> <th>번호</th><th width="300">제목</th><th>이름</th><th>작성일</th><th>조회수</th></tr>
<%
for(BoardVO m:li){
	int num=xidx+1;
	xidx++;
...
<a href="${path}/BoardController?sw=MS&xidx=0&str1=<%=str1 %>&str2=<%=str2 %>"> 처음 </a> &emsp;&emsp;
...
%>
<a href="${path}/BoardController?sw=MS&xidx=<%=beforePage %>&str1=<%=str1 %>&str2=<%=str2 %>"> 이전<%=pageListSize %> </a> 
<%} else { %>
 이전<%=pageListSize %>
<%} %>
<!-- *** 이전페이지 끝 *** -->
...
%>
 ...
 <a href="${path}/BoardController?sw=MS&xidx=<%=xidx %>&str1=<%=str1 %>&str2=<%=str2 %>">[<%=i%>]</a> &nbsp;
...
%>
<a href="${path}/BoardController?sw=MS&xidx=<%=nextPage %>&str1=<%=str1 %>&str2=<%=str2 %>"> 다음<%=pageListSize %> </a> &emsp;&emsp;
<% } else { %>
...
<a href="${path}/BoardController?sw=MS&xidx=<%=endIdx%>&str1=<%=str1 %>&str2=<%=str2 %>"> 마지막 </a> &emsp;&emsp;
